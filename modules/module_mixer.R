  ...
  output$uiMixer <- renderUI({
    f7Card(title = 'Location: CIC, 245 Main Street',
           tagList(
             ...
             hr(),
             p('Mixer Thursday, August 22nd 6:00pm-7:30pm'),
             p('CIC Building, 245 Main Street'),
             p('Enter just outside Kendall T Station and take elevator to 2nd floor to check in with concierge'),
             p('R/Pharma event on 3rd floor'),
             p('Please remember to bring your conference badge')
           )
           )
  })
}
